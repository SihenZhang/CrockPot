...
public final class CrockPotRegistry {
    ...
    public static RegistryObject<Item> stuffedEggplant = ITEMS.register("stuffed_eggplant", () -> CrockPotFood.builder().hunger(7).saturation(0.6F).heal(2.0F).duration(FoodUseDuration.FAST).build());
    public static RegistryObject<Item> surfNTurf = ITEMS.register("surf_n_turf", () -> CrockPotFood.builder().hunger(8).saturation(1.2F).setAlwaysEdible().effect(Effects.REGENERATION, 2 * 30).heal(8.0F).build());
    public static RegistryObject<Item> taffy = ITEMS.register("taffy", () -> CrockPotFood.builder().hunger(5).saturation(0.4F).duration(FoodUseDuration.FAST).effect(Effects.LUCK, 8 * 60 * 20).damage(CrockPotDamageSource.TAFFY, 1.0F).removePotion(Effects.POISON).build());
